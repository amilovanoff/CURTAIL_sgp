...
#Passenger activity by mode

```{r transport_activity_pkt,fig.height=3.5,fig.width=4}
modelframework::load_input_data_f()
...
plot <- ggplot()+
  geom_col(data=plot_dt,
   ...
   scale_fill_milovanoff(palette_type="cat",
                        ...
                        guide=guide_legend(title="Mode",nrow=1))+
  scale_y_continuous(expand = c(0.01,0))+
  scale_x_continuous(expand = c(0.01,0),breaks = c(2005,2010,2015,2019))+
  labs(x=NULL,y="Passenger activity (billion pkt)")+
  theme_milovanoff(legend_position = "right")

ggsave("outputs/plots/conference/hist_pkt.png",plot+theme(legend.position = "none"),width=4,height=3.5,units=c("in"),dpi=600)


plot_legend <- get_legend(plot)
ggsave("outputs/plots/conference/hist_pkt_leg.png",plot_legend,width=5,height=0.5,units=c("in"),dpi=600)

```


#Historical GHG emissions of passenger transport by mode- 2005-2019
...
plot <- ggplot()+
  ...
  theme_milovanoff(axis.title.y = element_text(angle=90,hjust = 0.5,vjust=0.5,face="bold"),
                   legend_position = "bottom",
                   legend.key.height = unit(0.4, "cm"),
                   legend.key.width = unit(0.4, "cm"),
                   legend.margin = margin(t=-0.2,r=0.5,b=0.1,l=0.5, "cm"))

ggsave("outputs/plots/conference/ghg_passenger_hist_mode.png",plot+theme(legend.position = "none"),width=4,height=3.5,units=c("in"),dpi=600)

```

#GHG emissions by life cycle stage

```{r transport_lca_ghg_stage_2005,fig.height=3.5,fig.width=5}
modelframework::load_input_data_f()
res <- do.call(transport_lca_ghg_f,list())
dt <- subset(res[["transport_lca_ghg_process"]],Year%in%c(2019) & Value!=0 & Process!="CNG") #REMOVE CNG 
#Format
...
plot_col$Category <- factor(plot_col$Category,levels=c("Vehicle prod.","Electricity prod.","Diesel prod.","Gasoline prod.","Diesel use","Gasoline use"))
...
ggplot(data=subset(plot_col))+
  ...
  guides(fill = guide_legend(title=NULL,nrow=2,reverse = TRUE))+
  coord_flip(clip="off",ylim=c(0,100))+
  scale_fill_manual(values=c("#bdbdbd","#d8b365","#A6CEE3","#FB9A99","#1F78B4","#E31A1C"))+
  scale_y_continuous(expand = c(0.01,0),labels = paste0(seq(0,100,25),"%"))+
  scale_x_discrete(expand = c(0.01,0))+
  labs(y="Stage contribution analysis",x=NULL)+
  theme_milovanoff(axis.title.x=element_text(hjust=0.5,face="plain",margin = unit(c(0,0,0,0),"mm")),
                   legend_position = "bottom",
                   legend.key.height = unit(0.4, "cm"),
                   legend.key.width = unit(0.4, "cm"),
                   legend.margin = margin(t=-0.2,r=0.5,b=0.1,l=-0.5, "cm"),
...
ggsave("outputs/plots/conference/ghg_passenger_hist_stage.png",width=5,height=3.5,units=c("in"),dpi=600)
...
```{r transport_lca_ghg_baseline,fig.height=4,fig.width=3}
#
...
ggsave("outputs/plots/conference/ghg_passenger_pros_line.png",width=3,height=4,units=c("in"),dpi=600)
...
```{r transport_lca_ghg_cum_phase,fig.height=2.5,fig.width=4}
#GHG emissions
res <- do.call(read_def_outputs_f,list(function_tbc="transport_lca_ghg_f",scen_tbc="scen1"))
...
#Format
plot_dt$Scenario <- factor(plot_dt$Scenario,levels=rev(c("PC_Constant","PC_EV","PT_Constant","PT_EV")))


ggplot()+
  geom_col(data=plot_dt,
           aes(x=Scenario,y=Value/10^9,fill=Phase))+
  geom_hline(data=budget_dt,
               aes(yintercept=Value/10^9,linetype=Scenario),
               size=1.2)+
  coord_flip()+
  scale_y_continuous(expand = c(0.01,0),limits=c(0,200))+
  scale_fill_manual(values=c("#bdbdbd","#1F78B4","#FB9A99","#E31A1C"),guide = guide_legend(title=NULL,nrow=2,order=1))+
  scale_linetype_manual(values=c(1,3),guide="none",labels=c("1.5C","2C"))+
  labs(x=NULL,y=expression("2018-2050 Cumulative emissions (Mt CO"[2]*" eq.)"))+
  theme_milovanoff(legend_position = "bottom",
                   legend.box.margin = margin(t=-0.5,r=0,b=0,l=-0.5, "cm"),
                   axis.text.x = element_text(face=,size=, angle=0,vjust=0.5,hjust=0.5),
                   axis.title.y = element_text(hjust=1),
                   axis.text.y = element_blank(),
                   plot.margin=unit(c(0,0,0,0),"mm"),
                   legend.box.just = "left")

ggsave("outputs/plots/conference/ghg_passenger_pros_phase_cum.png",width=4,height=2.5,units=c("in"),dpi=600)
...
#Calculate degrowth rate
de_rate <- subset(dt_per_cap,Year==2020)
de_rate$degrowth_rate <- sapply(1:nrow(de_rate),function(x) 1- de_rate[x,"Value_per_cap"]/subset(de_rate,Scenario=="Default" & Target==de_rate[x,"Target"])$Value_per_cap)
...
ggsave("outputs/plots/conference/ev_elec.png",width=4,height=4,units=c("in"),dpi=600)
