...
#Private car
mat_car_pop <- as.matrix(read.csv("inputs/data/car_population_by_age.csv",stringsAsFactors = FALSE,check.names = FALSE,row.names = 1))
...
mat_car_survival <- mat_car_survival[as.character(1:14),]
#Force negative values t 0
mat_car_survival[mat_car_survival>1] <- 1
#Calculate the lowest, median and highest distribution
cum_survival <- sapply(colnames(mat_car_survival),function(x)cumprod(mat_car_survival[,x]))["14",as.character(2005:2018)]
min_dis_yr <- as.character(2005:2018)[which.min(cum_survival)]
max_dis_yr <- as.character(2005:2018)[which.max(cum_survival)]
def_dis_yr <- as.character(2005:2018)[which.min(abs(cum_survival-median(cum_survival)))]
...
mat_car_survival[mat_car_survival>1] <- 1
#Calculate the lowest, median and highest distribution
cum_survival <- sapply(colnames(mat_car_survival),function(x)cumprod(mat_car_survival[,x]))["19",as.character(2006:2017)]
min_dis_yr <- as.character(2006:2017)[which.min(cum_survival)]
max_dis_yr <- as.character(2006:2017)[which.max(cum_survival)]
def_dis_yr <- as.character(2006:2017)[which.min(abs(cum_survival-median(cum_survival)))]
