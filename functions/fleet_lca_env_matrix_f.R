...
fleet_lca_env_matrix_f <- function(mode,first_yr=NA,last_yr=NA,ef_elec_scen=NA,fast_mode="n"){
  ...
  lca_env_matrix[lca_process$Process=="Battery production and assembly",] <- subset(EF_lit,`Battery type`=="Li_ion LMO" & Process=="Battery production" & Modele=="def")$LCI
  ...
  for (i in which(lca_process$Source=="Asian Development Bank")){
    lca_env_matrix[i,] <- as.numeric(subset(ef_mobile_combustion,Fuel==lca_process[i,"Process"] & Unit=="kg CO2 / Liter")[,"Emission Factor"])
  }
  ...
  if (ef_elec_scen=="constant"){
    lca_env_matrix[which(lca_process$Phase=="Fuel Production" & lca_process$Process=="Electricity"),] <- 0.425
  } else if(ef_elec_scen=="renewable"){
    lca_env_matrix[which(lca_process$Phase=="Fuel Production" & lca_process$Process=="Electricity"),as.character(2005:2019)] <- 0.425
    lca_env_matrix[which(lca_process$Phase=="Fuel Production" & lca_process$Process=="Electricity"),as.character(2020:2030)] <- approx(x=c(2019,2030), y=c(0.425,0.425*0.5) , xout=2020:2030, method = "linear")$y
  }
