...
#' @export
vehicle_specifications_f = function(vehicle,fc_impro=NA,last_yr=NA){
  ...
  vehicle$specifications <- matrix(NA,nrow=length(specs_list),ncol=last_yr-i_year+1,dimnames=list(specs_list,i_year:last_yr))
  ...
  vehicle$specifications["peak_power",as.character(first_cpt_composition_yr)] <- sum(cpt_dt$peak_power)
  #Update battery density and vehicle range
  if ("Electricity"%in%vehicle$fuel_type){
    #Battery density
    vehicle$specifications["battery_density",as.character(i_year:last_yr)] <- do.call(battery_density_f,list(vehicle=vehicle,model_year=i_year:last_yr))
    #Initial vehicle range from battery weight
    #tmp_techno is the techno without range value
    tmp_techno <- substring(vehicle$technology,0,as.numeric(regexpr(pattern="[[:digit:]]{1}",vehicle$technology))-1)
    i_dens <- vehicle$specifications["battery_density",as.character(first_cpt_composition_yr)]
    ...
    elec_cons <- vehicle$fuel_consumption["Electricity",as.character(first_cpt_composition_yr)]
    ...
    vehicle$specifications["range",as.character(first_cpt_composition_yr)] <- i_dens*wgt_bat/elec_cons*100*usable_en
  }
  #Update vehicle range
  return(vehicle)
}
