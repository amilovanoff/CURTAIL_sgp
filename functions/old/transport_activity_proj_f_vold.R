...
transport_activity_proj_f <- function(transport,first_yr=NA,last_yr=NA,pkt_proj_tot_scen=NA,pkt_tot_variable=NA,pkt_proj_modal_share_scen=NA,modal_share_variable=NA){
  ...
  if (pkt_proj_modal_share_scen=="constant"){
    ...
    for (mode in mode_pt){
      #Absolute
      transport$pkt[mode,as.character(first_proj_yr:2030)] <-  transport$pkt[mode,as.character(first_proj_yr-1)]+sapply(first_proj_yr:2030, function(x)(x-first_proj_yr+1)*(transport$pkt[mode,as.character(f_year_trend)]-transport$pkt[mode,as.character(i_year_trend)])/(f_year_trend-i_year_trend))
      #Relative
    ...
    mode_tbc <- c("MRT",mode_pt,mode_tbc)
    ...
    #Assume constant annual mileage for all
    transport$kt_per_veh[c(mode_tbc,"Private car"),as.character(first_proj_yr:last_yr)] <- transport$kt_per_veh[c(mode_tbc,"Private car"),as.character(first_proj_yr-1)]
