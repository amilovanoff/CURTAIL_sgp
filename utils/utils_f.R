...
#' fun_res_f
#'
#' Return the results of function. First check if function results are saved in local or saved environment. If fast mode, read the default results.
#' @param fun_name Function name
#' @param fast_mode Shall it extract default simulation results?
#' @export
fun_res_f <- function(fun_name,
  ...
  if (exists(paste0(fun_name,"_res"),where = fun.res.env)){
    ...
    if (fast_mode=="y"){
      #Get list of scenario attributes with values.
      scen_attributes <- setNames(list(get_scenario_attributes("def")[,"Default"]),"Scenario")
      ...
      #Only consider the results with the same attribute values than the scenario attribute values
      temp_res <- get("results_l",envir = fun.res.env)[[sapply(seq_len(length(get("results_l",envir = fun.res.env))),function(x)scen_attributes %in% get("results_l",envir = fun.res.env)[[x]])]]
      assign(paste0(fun_name,"_res"),temp_res[names(temp_res)!="Scenario"],envir = fun.res.env)
      fun_res <- get(paste0(fun_name,"_res"), envir = fun.res.env)
...
fun_res_with_attributes <- function(fun_name,
  ...
  return(do.call(fun_res_f,list(fun_name=fun_name,fast_mode=fast_mode)))
}
...
#' @import readxl
#' @export
get_scenario_attributes <- function(scen_tbc){
  scenarios <- as.data.frame(read_xlsx("outputs/scenarios.xlsx",sheet="scenarios",.name_repair = "minimal"))
...
#' @import readxl
#' @export
get_discrete_simulation_attributes <- function(sim_tbc){
  simulations <- as.data.frame(read_xlsx("outputs/simulations.xlsx",sheet = "discrete",.name_repair = "minimal"))
  #Fill empty spaces with NA
...
#' @import readxl
#' @export
get_continuous_simulation_attributes <- function(sim_tbc){
  simulations <- as.data.frame(read_excel("outputs/simulations.xlsx",sheet = "continuous",.name_repair = "minimal"))
  #Fill empty spaces with NA
  simulations[simulations==""] <- NA
  #Delete rows not associated with sim_tbc
  sim_attribute_value <- subset(simulations, Simulation==sim_tbc)
  ...
  for (sim_name in sim_names){
    ...
    cont_attr_values <- sapply(colnames(seq_par),function(x)seq(from=seq_par["from",x],to=seq_par["to",x],by=seq_par["by",x]))
...
#' @export
get_simulation_attribute_to_keep <- function(sim_tbc,
                                             sim_type="discrete"){
  simulations <- as.data.frame(read_excel("outputs/simulations.xlsx",sheet = sim_type,.name_repair = "minimal"))
  #Fill empty spaces with NA
  simulations[simulations==""] <- NA
  #Delete rows not associated with sim_tbc
  sim_attribute_value <- subset(simulations, Simulation==sim_tbc)
...
#' @import readxl
#' @export
get_sens_analysis_attributes <- function(sens_tbc,
                                         function_tbc){
  if (sens_tbc!="complete"){
    sens_analysis <- as.data.frame(read_excel("outputs/sens_analysis.xlsx",sheet = "single_factor",.name_repair = "minimal"))
...
#' @import readxl
#' @export
get_mf_sens_analysis_attributes <- function(mf_sens_tbc){
    sens_analysis <- as.data.frame(read_excel("outputs/sens_analysis.xlsx",sheet = "multi_factor",.name_repair = "minimal"))
    #Get attributes
...
#' @param get_default_attr_val Shall it get the default value?
#' @export
get_attribute_description <- function(attribute){
  return(subset(get("attribute_value", envir = fun.att.env),Attribute==attribute)[,"Description"])
}
